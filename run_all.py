...
if __name__ == '__main__':
    ...
    results_dir = os.path.join(os.path.dirname(args.dataset_dir), f'results')
    os.makedirs(results_dir, exist_ok=True)
    
    trimap_path = os.path.join(args.dataset_dir, 'trimap/')
    img_paths = glob.glob(os.path.join(args.dataset_dir, 'images/*'))
    
    for img_path in tqdm.tqdm(img_paths):
        # Gets file name
        image_name = os.path.split(img_path)[-1] 
        input = get_data(img_path, os.path.join(trimap_path,image_name))
        ...
        alpha = infer_one_image(model, input)
        print('Inferencing finished.')
        
        torch.cuda.empty_cache()
        input_x = Image.open(img_path)
        ...
        out_path = os.path.join(results_dir, os.path.splitext(os.path.basename(img_path))[0] + '.png')
        foreground_alpha.save(out_path)
